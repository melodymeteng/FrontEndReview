...
const createStore = function(reducer, initState, rewriteCreateStoreFunc) {
  ...
  function dispatch(action) {
    // console.log('actipon',state,action)
...
//bindActionCreators 通过闭包把dispatch和actionCreators隐藏起来
...
function bindActionCreators(actionCreators, dispatch) {
  if (Object.prototype.toString.call(actionCreators) === '[object Function]') {
  ...
  if (
    ...
    return new TypeError('不是函数或对象或者是null');
  }
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (Object.prototype.toString.call(actionCreator) === '[object Function]') {
      actionCreatorsObj[key] = bindActionCreator(actionCreator, dispatch);
    }
  }
  return actionCreatorsObj;
}
const actions = bindActionCreators({ addDis, setnameDis }, store.dispatch);
// actions.addDis();
actions.setnameDis('goudan');
